...
public class CarAI : MonoBehaviour
{
    ...
    [Header("Dynamic Destination")]
    public float repathDistance = 3f; // quanto o destino precisa se mover pra recalcular
    private Vector3 lastDestinationPos;

    [Header("Unstuck parameters")]
    ...
    void Start()
    {
            ...
            {
                lastDestinationPos = CustomDestination.position;
            }
    ...
    void Update()
    {
        ...
        bool playerInFOV = IsTargetInFOV(PlayerTarget);

        if (playerInFOV && CustomDestination != PlayerTarget)
        {
            CustomDestination = PlayerTarget;
            Patrol = false;
            SwitchMode();
            lastDestinationPos = PlayerTarget.position;
        }
        else if (!playerInFOV && CustomDestination != null)
        {
        ...
        if (!Patrol && CustomDestination != null)
        {
            float distanceMoved = Vector3.Distance(lastDestinationPos, CustomDestination.position);

            if (distanceMoved >= repathDistance)
            {
                RecalculatePathToMovingTarget();
                lastDestinationPos = CustomDestination.position;
            }
        }
    }
    ...
    bool IsTargetInFOV(Transform target)
    {
        Vector3 dirToTarget = (target.position - carFront.position).normalized;
        float angle = Vector3.Angle(carFront.forward, dirToTarget);

        return angle <= AIFOV * 0.5f;
    }
    ...
    void CheckIfStuck()
    {
        ...
        if (distanceMoved < minMoveDistance && speed < minSpeed)
        {
            stuckTimer += Time.fixedDeltaTime;

            if (stuckTimer >= timeToUnstuck)
            {
                UnstuckCar();
                stuckTimer = 0f;
            }
    ...
    void RecalculatePathToMovingTarget()
    {
        waypoints.Clear();
        currentWayPoint = 0;
        allowMovement = true;
    ...
    {
        frontLeft.brakeTorque = 1500;
        frontRight.brakeTorque = 1500;
        backLeft.brakeTorque = 1500;
        backRight.brakeTorque = 1500;
    }
    ...
    void ApplyReverse()
    {
        backLeft.motorTorque = -MaxRPM;
        backRight.motorTorque = -MaxRPM;
    }
    ...
    public void BrakeFor(float duration)
    {
