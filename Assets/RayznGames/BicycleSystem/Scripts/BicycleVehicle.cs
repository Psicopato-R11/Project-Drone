...
namespace rayzngames
{
	...
	public class BicycleVehicle : MonoBehaviour
	{
        ...
        [Range(10, 350)] public int maxSpeed = 200;
        [Range(1, 30)] public int accelerationMultiplier = 2;
		...
		[Tooltip("Adjust this parameter to add an offset to the center of gravity")]
		public Vector3 COG;
		...
		[SerializeField] public float currentSteeringAngle { get; private set; }
		[Tooltip("Dynamic MAX steering angle based on the linearspeed of the RB, affected by sterReductorAmmount")]
		[SerializeField] public float current_maxSteeringAngle { get; private set; }
		...
		private void HandleEngine()
		{
            ...
            // Calcula a velocidade local no eixo Z (frente/trás)
            ...
            {
                // LOGICA DO CARRO: Se estiver indo para trás (Z < -1) e acelerar, freia primeiro
                if (localVelocityZ < -1f && verticalInput > 0)
                {
                    ApplyBraking(brakeForce);
                ...
                else
                {
                    ...
                    if (Mathf.RoundToInt(speedInKmh) < maxSpeed)
                    {
                        ApplyBraking(0f);
                        // Aplica o torque usando a fórmula do seu carro: (Multiplicador * 50) * Input
                        // Como moto só tem 1 roda motriz, você pode testar aumentar o 50f para 100f se achar lenta
                        rearWheel.motorTorque = (accelerationMultiplier * 50f) * verticalInput;
                    }
                    else
                    {
                        rearWheel.motorTorque = 0;
            ...
            // Lógica de Desaceleração (DecelerateCar)
            // Se o jogador soltar o acelerador, aplica uma pequena frenagem ou reduz torque
            if (verticalInput == 0 && !braking)
            {
                rb.linearVelocity = rb.linearVelocity * (1f / (1f + (0.005f * decelerationMultiplier)));

                // 2. Remove o torque das rodas (como no seu script original)
                rearWheel.motorTorque = 0f;
                frontWheel.motorTorque = 0f;

                // 3. Aplica uma frenagem leve para evitar que a moto role em ladeiras
                // (Ajuste o 5f se quiser que ela role mais ou menos)
                ApplyBraking(decelerationMultiplier * 2f);
            }
        }
